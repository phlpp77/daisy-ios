...
//  Meet Me
//
//  Created by Philipp Hemkemeyer on 26.03.21.
//
...
struct ImageCroppingView: View {
    ...
    // closure which returns true if image is cropped
    var didCroppedImage: (_ imageCropped: Bool) -> Void
        ...
        ZStack {
            ...
            ZStack {
                Image(uiImage: image)
                    ...
                    .scaledToFit()
                    .overlay(
                        // only to get the figures for calculation
                        GeometryReader { bounds -> AnyView in
    ...
    func cropImage() {
        ...
        let xOffset = (((imageWidth/2) - (getDimension(w: imageWidth, h: imageHeight) * croppingMagnification/2)) + croppingOffset.width) * scale
        let yOffset = (((imageHeight/2) - (getDimension(w: imageWidth, h: imageHeight) * croppingMagnification/2)) + croppingOffset.height) * scale
...
struct ViewFinderView:View{
    ...
    var body: some View {
        ZStack{
            // MARK: Surrounding rectangles of the cropped image view
            Group{
                // left side
                Rectangle()
                ...
                .gesture(
                    DragGesture()
                        .onChanged{drag in
                            ...
                            //Here we are setting up the upper and lower limits of the magnification
